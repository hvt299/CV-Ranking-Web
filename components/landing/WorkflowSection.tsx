...
export default function WorkflowSection() {
    return (
        <section id="workflow" className="py-32 px-6 bg-background dark:bg-slate-950 border-y border-slate-200 dark:border-slate-800 scroll-mt-20 transition-colors">
            <div className="max-w-7xl mx-auto text-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-12 relative">
                    <div className="hidden md:block absolute top-12 left-1/6 right-1/6 h-px bg-slate-300 dark:bg-slate-700 z-0" />
                    ...
                    {WORKFLOW_STEPS.map((item, i) => (
                        <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.2 }} className="relative z-10 flex flex-col items-center">
                            <div className="w-24 h-24 bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-700 rounded-full flex items-center justify-center text-2xl font-black text-blue-600 dark:text-blue-400 mb-6 shadow-xl transition-colors">
                                {item.step}
                            </div>
