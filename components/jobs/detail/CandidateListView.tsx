...
export default function CandidateListView({ candidates, expandedInsights, isGeneratingInterview, onStatusChange, onToggleView, onViewCV, onAddNote, onRemoveFromJob, onToggleInsightExpand, onViewSkills, onGenerateQuestions }: CandidateListViewProps) {
    const { data: plansRes } = useSubscriptionPlans('hr');
    const unlockInterviewPlan = plansRes?.data?.find((p: any) => p.tier_level >= 2);
                    ...
                    <div key={cv.id} className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md hover:border-primary-300 dark:hover:border-primary-700 transition-all group">
                        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start xl:items-center">
                                ...
                                <div className="flex-1 w-full min-w-0">
                                    ...
                                    {aiSentences.length > 0 && (
                                        <div className="bg-amber-50/70 dark:bg-slate-900/50 p-3 rounded-lg border border-amber-100 dark:border-slate-700 text-xs text-slate-600 dark:text-slate-400 flex items-start gap-2 relative">
                                            ...
                                            <span className={`px-1.5 py-0.5 rounded-sm text-[9px] font-black uppercase tracking-widest shadow-sm shrink-0 mt-0.5 border ${badgeConfig.bg} ${badgeConfig.text} ${badgeConfig.border}`}>{unlockInterviewPlanName.replace('HR ', '')}</span>
                                            ...
                                            <div className="flex-1">
                                                ...
                                                {aiSentences.length > 2 && (
                                                    <button onClick={() => onToggleInsightExpand(cv.id)} className="text-[10px] text-amber-600 font-bold hover:underline mt-2 inline-block">
                                    ...
                                    <div className="mt-3 flex justify-between items-center">
                                        <button onClick={() => onGenerateQuestions(cv)} disabled={isGeneratingInterview === cv.id} className="flex items-center gap-1.5 text-[11px] font-bold text-primary-600 bg-primary-50 hover:bg-primary-100 dark:bg-primary-900/20 dark:hover:bg-primary-900/40 px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50 shadow-sm">
                            ...
                            <div className="xl:col-span-2 flex flex-row xl:flex-col justify-between items-center xl:items-end border-t xl:border-t-0 xl:border-l border-slate-100 dark:border-slate-700 pt-4 xl:pt-0 xl:pl-6 h-full w-full">
                                <select className={`w-1/2 xl:w-full text-xs font-bold px-3 py-2 rounded-xl outline-none cursor-pointer text-center transition-colors ${APPLICATION_STATUS_CONFIG[cv.status as ApplicationStatus]?.color || 'bg-slate-100 text-slate-700'}`} value={cv.status || ApplicationStatus.NEW} onChange={(e) => onStatusChange(cv.id, e.target.value, cv)}>
