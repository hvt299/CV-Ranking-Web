            ...
            <div className="sticky bg-slate-50/90 dark:bg-[#0f172a]/90 backdrop-blur-md pb-4 pt-4 mb-6 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-500 font-medium">Hệ thống phân tích NLP & Hybrid Scoring</p>
                    </div>
                </div>
                <div className="flex gap-3">
                    ...
                    <button type="submit" disabled={isLoading} className="px-6 py-2.5 rounded-xl font-bold text-white bg-blue-600 hover:bg-blue-700 shadow-lg flex items-center gap-2">
                        <Save className="w-5 h-5" /> {isLoading ? 'Đang lưu...' : 'Xuất bản Chiến dịch'}
                    </button>
                </div>
            </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-indigo-50 dark:bg-indigo-500/5 rounded-3xl p-6 border border-indigo-100 dark:border-indigo-500/20 shadow-sm">
                        ...
                        <div className="space-y-4 mb-6 pb-6 border-b border-indigo-100 dark:border-indigo-500/20">
                            ...
                            <div>
                                <label className="block text-sm font-bold mb-2 text-slate-700 dark:text-slate-300">Học vấn tối thiểu</label>
                                <select
                            ...
                            <div>
                                <label className="block text-sm font-bold mb-2 text-slate-700 dark:text-slate-300">Chuyên ngành ưu tiên</label>
                            ...
                            <div className="space-y-2">
                                {requiredSkills.map((skill, index) => (
                                    <div key={index} className="bg-white dark:bg-slate-800 p-2 rounded-lg border border-slate-200 dark:border-slate-700 flex gap-2 items-center">
                                        ...
                                        <input type="number" min="0" step="0.1" className="w-1/4 p-1.5 bg-slate-50 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500 rounded outline-none text-sm text-center" value={skill.weight} onChange={e => handleSkillChange('required', index, 'weight', Number(e.target.value))} />
                                        <input type="number" min="0" step="0.5" className="w-1/4 p-1.5 bg-slate-50 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500 rounded outline-none text-sm text-center" value={skill.min_years} onChange={e => handleSkillChange('required', index, 'min_years', Number(e.target.value))} />
                            ...
                            <div className="space-y-2">
                                {preferredSkills.map((skill, index) => (
                                    <div key={index} className="bg-white dark:bg-slate-800 p-2 rounded-lg border border-slate-200 dark:border-slate-700 flex gap-2 items-center">
                                        ...
                                        <input type="number" min="0" step="0.1" className="w-1/4 p-1.5 bg-slate-50 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500 rounded outline-none text-sm text-center" value={skill.weight} onChange={e => handleSkillChange('preferred', index, 'weight', Number(e.target.value))} />
                                        <input type="number" min="0" step="0.5" className="w-1/4 p-1.5 bg-slate-50 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500 rounded outline-none text-sm text-center" value={skill.min_years} onChange={e => handleSkillChange('preferred', index, 'min_years', Number(e.target.value))} />
