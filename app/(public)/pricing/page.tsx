                ...
                <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
                    ...
                    <div className="mt-8 inline-flex items-center p-1.5 bg-slate-100 dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700">
                        <button
                            ...
                            className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold transition-all ${audience === 'hr' ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
                        >
                            <Building2 className="w-4 h-4" /> Nhà tuyển dụng
                        </button>
                        <button
                            onClick={() => setAudience('applicant')}
                            className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold transition-all ${audience === 'applicant' ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
                        >
                ...
                {isLoading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-stretch">
                        {plans.map((plan: any) => {
                            ...
                            const isCurrentPlan = myPlan?.current_plan_code === plan.plan_code;
                            const isUpgrade = myPlan && plan.current_price > 0 && plan.plan_code !== myPlan.current_plan_code;

                            return (
                                <div key={plan.id} className={`relative flex flex-col h-full bg-white dark:bg-[#0a0a0a] rounded-3xl p-6 shadow-sm transition-all duration-300 ${isHighlighted ? 'border-2 border-blue-500 shadow-blue-500/10 md:-translate-y-2' : 'border border-slate-200 dark:border-slate-800'}`}>
                                    {isHighlighted && (
                                        <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-blue-500 text-white text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full flex items-center gap-1 shadow-md whitespace-nowrap">
                                    ...
                                    {isCurrentPlan ? (
                                        <button disabled className="w-full py-3 rounded-xl font-bold flex items-center justify-center transition-colors text-sm bg-emerald-50 text-emerald-600 border border-emerald-200 cursor-not-allowed">
                                            Đang sử dụng
                                        </button>
                                    ) : (
                                        <Link href={isAuthenticated ? `/${audience}/settings/billing?plan=${plan.plan_code}` : `/register`}
                                            className={`w-full py-3 rounded-xl font-bold flex items-center justify-center transition-colors text-sm ${isHighlighted ? 'bg-blue-600 hover:bg-blue-700 text-white shadow-md shadow-blue-500/20' : 'bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700'}`}>
