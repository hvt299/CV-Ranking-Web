...
export default function RegisterPage() {
    ...
    const [socialHrInfo, setSocialHrInfo] = useState({ companyName: '', taxCode: '', industry: '', size: '', address: '', website: '' });
    ...
    const handleLookupTax = async (code: string, isSocialForm: boolean = false) => {
        ...
        try {
            ...
            if (isSocialForm) {
                ...
                setHrInfo(prev => ({ ...prev, companyName: res.data.company_name, address: res.data.address || '' }));
            }
            toast.success("Đã tìm thấy thông tin công ty!");
        } catch (e: any) {
            toast.error(e.response?.data?.detail || "Không tìm thấy dữ liệu");
        }
    };
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-[#0f172a] flex items-center justify-center p-4 py-10 transition-colors duration-300">
            <div className="max-w-xl w-full bg-white dark:bg-[#1e293b] rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-none p-8 md:p-10 border border-slate-100 dark:border-slate-800">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input type="text" required value={fullName} onChange={e => setFullName(e.target.value)} className="w-full pl-9 pr-3 py-2.5 bg-slate-50 dark:bg-[#0f172a] border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-blue-500 text-sm" placeholder="Nguyễn Văn A" />
                            </div>
                        </div>
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input type="email" required value={email} onChange={e => setEmail(e.target.value)} className="w-full pl-9 pr-3 py-2.5 bg-slate-50 dark:bg-[#0f172a] border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-blue-500 text-sm" placeholder="name@company.com" />
                            </div>
                        </div>
                    </div>
                    ...
                    <button type="submit" disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 mt-4">
            ...
            {showRoleModal && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white dark:bg-[#1e293b] p-6 md:p-8 rounded-3xl w-full max-w-lg shadow-2xl overflow-y-auto max-h-[90vh]">
                        ...
                        <button onClick={submitSocialRole} disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-bold">
