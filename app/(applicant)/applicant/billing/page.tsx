...
import { Zap, CheckCircle2, History, Loader2, ShieldCheck } from 'lucide-react';
...
export default function ApplicantBillingPage() {
    const queryClient = useQueryClient();
    const { data: myPlanRes, isLoading: isMyPlanLoading } = useSubscription();
    const { data: plansRes, isLoading: isPlansLoading } = useSubscriptionPlans('applicant');
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-32">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
                <div>
                    <h1 className="text-2xl font-black text-slate-800 dark:text-white tracking-tight">Nâng cấp Hồ sơ</h1>
                    <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Sử dụng sức mạnh AI để làm nổi bật CV, gia tăng 300% cơ hội trúng tuyển.</p>
                </div>
            </div>
            ...
            <div className="bg-primary-50/50 dark:bg-primary-900/10 border-2 border-primary-200 dark:border-primary-800/50 rounded-3xl p-8 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-64 h-64 bg-primary-500/10 blur-3xl rounded-full translate-x-1/3 -translate-y-1/3 pointer-events-none" />
                <div className="relative z-5 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                    <div>
                        <p className="text-slate-500 dark:text-slate-400 font-bold text-xs uppercase tracking-wider mb-1">Gói Tài khoản hiện tại</p>
                        <h2 className="text-3xl font-black text-slate-800 dark:text-white flex items-center gap-2">
                            {planInfo?.current_plan || 'Đang tải...'}
                            {planInfo?.current_plan_code !== 'app_free' && (
                    ...
                    <div>
                        <p className="text-slate-500 dark:text-slate-400 font-bold text-xs uppercase tracking-wider mb-1">Credit AI (Sửa CV, Phân tích)</p>
            ...
            <div id="pricing-grid" className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {plans.map((plan: any) => {
                    ...
                    return (
                        <div
                            key={plan.plan_code}
                            className={`relative p-6 rounded-3xl border-2 transition-all flex flex-col ${isCurrent
                                ? 'border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 opacity-60'
                                : isHighlighted
                                    ? 'border-amber-400 shadow-sm bg-white dark:bg-slate-900'
                                    : 'border-slate-200 dark:border-slate-800 hover:border-primary-300 dark:hover:border-primary-700 bg-white dark:bg-slate-900'
                                }`}
                        >
                            ...
                            <div className="mb-4">
                                <p className="text-2xl font-black text-slate-800 dark:text-white">
                                    ...
                                    <span className="text-sm font-medium text-slate-500 dark:text-slate-400">{plan.current_price > 0 && ' VNĐ'}</span>
                                </p>
                            ...
                            >
                                {isCurrent ? 'Đang kích hoạt' : processingPlanCode === plan.plan_code ? (
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col">
                <div className="p-6 border-b border-slate-100 dark:border-slate-800">
                    <h3 className="text-lg font-black text-slate-800 dark:text-white flex items-center gap-2">
                        <History className="w-5 h-5 text-primary-500" /> Lịch sử Giao dịch
                    </h3>
                </div>
                                ...
                                transactions.map((tx: any) => {
                                    ...
                                    return (
                                        <tr key={tx.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors">
                                            <td className="p-4 pl-6 text-sm text-slate-600 dark:text-slate-300 font-medium whitespace-nowrap">{new Date(tx.created_at).toLocaleString('vi-VN')}</td>
                                            <td className="p-4 text-sm font-bold text-slate-800 dark:text-white">{tx.action_type.replace('UPGRADE_', 'Nâng cấp gói: ')}</td>
                                            ...
                                            <td className="p-4 text-center pr-6 font-black text-slate-700 dark:text-slate-200">{tx.balance_after}</td>
                                        </tr>
                                    );
                                })
                            ) : (
                                <tr>
                                    <td colSpan={4} className="p-8 text-center text-slate-500 font-medium">Chưa có giao dịch nào phát sinh.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
